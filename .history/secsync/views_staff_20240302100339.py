...
def registerStaff(request, *args, **kwargs):
   ...
   if serializer.is_valid():
      phone_number = request.data['user_name'] 
      ...
      try:
         ...
         user = Staff.objects.get(email=email) #checking for email
      except Staff.DoesNotExist: 
         try:
            ...
            subject = 'Welcome to VoisekB2C'
            link = 'www.voisek.com'
            message = "Hi and welcome to VoisekB2C. You default password is "+ raw_password +" Please click on the link below to activate your account and setup a new password "+link
            sendEmail(sender_email, email, subject, message)
           
           
            
            response = {'status' : 'User was created successfully'}  
         except Exception as e:
            response = {'status': 'Error'}
         else:
            response = {'status': 'User Name is already registered'}
      except Exception as e:
         response = {'status': 'Error'}
      else:
